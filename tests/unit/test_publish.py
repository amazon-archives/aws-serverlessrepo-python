...
class TestPublishApplication(TestCase):
    def setUp(self):
        ...
        self.application_id = (
            "arn:aws:serverlessrepo:us-east-1:123456789012:applications/test-app"
        )
        self.application_exists_error = ClientError(
            {
                "Error": {
                    "Code": "ConflictException",
                    "Message": "Application with id {} already exists".format(
                        self.application_id
                    ),
                }
            },
        ...
        self.not_conflict_exception = ClientError(
            {"Error": {"Code": "BadRequestException", "Message": "Random message"}},
            "create_application",
        )
        ...
        self.invalid_s3_uri_exception = ClientError(
            {"Error": {"Code": "BadRequestException", "Message": "Invalid S3 URI"}},
            "create_application",
        )
        self.paginator_mock = Mock()
        self.paginator_mock.paginate.return_value = [
            {"ApplicationId": self.application_id, "Name": "test-app"}
        ]
        self.serverlessrepo_mock.get_paginator.return_value = self.paginator_mock
    ...
    def test_publish_raise_metadata_error_for_invalid_create_application_request(self):
        ...
        message = str(context.exception)
        self.assertEqual(
            "Invalid application metadata: 'name properties not provided'", message
        )
    ...
    def test_publish_raise_serverlessrepo_client_error_when_create_application(self):
        self.serverlessrepo_mock.create_application.side_effect = (
            self.not_conflict_exception
        )
    ...
    def test_publish_existing_application_should_update_application_if_version_not_specified(
        ...
        expected_request = dict(
            {"ApplicationId": self.application_id}, **expected_result["details"]
        )
    ...
    def test_publish_wrap_client_error_when_update_application(
        self, wrap_client_error_mock
    ):
        self.serverlessrepo_mock.create_application.side_effect = (
            self.application_exists_error
        )
        self.serverlessrepo_mock.update_application.side_effect = (
            self.not_conflict_exception
        )
    ...
    def test_publish_wrap_client_error_when_create_application_version(
        ...
        self.serverlessrepo_mock.create_application_version.side_effect = (
            self.not_conflict_exception
        )
        wrap_client_error_mock.return_value = ServerlessRepoClientError(
            message="client error"
    ...
    def test_update_application_with_passed_in_sar_client(self):
        ...
        sar_client.get_paginator.return_value = paginator_mock
    ...
    def test_update_application_with_readmebody(self):
        ...
        template_with_readmebody = self.template.replace(
            '"SemanticVersion": "1.0.0"', ""
        ).replace(
    ...
    def test_update_application_with_multiple_matching_applications(self):
        sar_client = Mock()
        paginator_mock = Mock()
        paginator_mock.paginate.return_value = [
            {"ApplicationId": self.application_id, "Name": "test-app"},
        ...
        with self.assertRaises(MultipleMatchingApplicationsError):
            publish_application(self.template, sar_client)
...
class TestUpdateApplicationMetadata(TestCase):
    ...
    @patch("serverlessrepo.publish.parse_template")
    def test_update_application_metadata_with_template_string_should_parse_template(
